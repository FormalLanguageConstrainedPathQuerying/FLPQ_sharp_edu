...
let ``BelyaninRPQ header in tikz mode orders legend before regexp before DFA before graph`` () =
    TestHelpers.withTempDir (fun dir ->
        ...
        // The Belyanin legend describes the frontier automaton states, not a tree node.
        Assert.Contains("Frontier automaton states", text)
...
let ``belyaninStepSection in dot mode fills empty placeholders for missing files`` () =
    TestHelpers.withTempDir (fun dir ->
        ...
        let template = "M=__MATRICES__|A=__STEP_AUTOMATON_PDF__|G=__STEP_GRAPH_PDF__"

        let lines = SummaryTeX.belyaninStepSection stepDir 0 template "" false
...
let ``belyaninStepSection in tikz mode fills empty placeholders for missing files`` () =
    TestHelpers.withTempDir (fun dir ->
        ...
        let template = "A=__STEP_AUTOMATON_TIKZ__|G=__STEP_GRAPH_TIKZ__"
