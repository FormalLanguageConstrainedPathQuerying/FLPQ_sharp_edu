...
module PathIndexGolden =
    ...
    let ``RNGLR path index TeX golden for S->aaA|aA, A->aA|eps, input aa`` () =
        ...
        TestHelpers.assertPathIndexInvariant "RNGLR golden" pathIndex (Some (extRsm.BlockStart |> Seq.map (fun kv -> kv.Key, kv.Value) |> Map.ofSeq)) (Some (RSM.blockFinalsMap extRsm))
        let tex = PathIndexTeX.toTeX string string pathIndex
...
module PathIndexCompilation =
    ...
    let ``RNGLR path index TeX compiles with lualatex`` () =
        ...
        let extRsm = RSM.extendWithStart freshStart rsmFixed
        let pathIndex = Rnglr.buildPathIndex freshStart rsmFixed graph
        TestHelpers.assertPathIndexInvariant "RNGLR compilation" pathIndex (Some (extRsm.BlockStart |> Seq.map (fun kv -> kv.Key, kv.Value) |> Map.ofSeq)) (Some (RSM.blockFinalsMap extRsm))
        let tex = PathIndexTeX.toTeX string string pathIndex
        Assert.True(ExternalTools.compileTexStringWithTemplate templatePath tex)
