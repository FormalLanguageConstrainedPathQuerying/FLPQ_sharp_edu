...
let private deriveVertexCount (edges: Trans<string> list) (extraVerts: int list) : int =
    let allVerts =
        edges |> List.collect (fun t -> [ t.From; t.To ]) |> List.append extraVerts

    if List.isEmpty allVerts then 0 else (List.max allVerts) + 1

let private nfaWithSources (edges: Trans<string> list) (sources: int list) : NFA<string, int> =
...
let ``Belyanin: v0-[a]->v1-[b]->v2, query a*b, v2 reachable`` () =
    let nfa =
        nfaWithSources [ { From = 0; Label = "a"; To = 1 }; { From = 1; Label = "b"; To = 2 } ] [ 0 ]

    let dfa =
        TestHelpers.buildDfa [ { From = 0; Label = "a"; To = 0 }; { From = 0; Label = "b"; To = 1 } ] 0 [ 1 ]
...
let ``Belyanin: v0-[a]->v1-[a]->v2, query a+, v1 and v2 reachable`` () =
    ...
    let dfa =
        TestHelpers.buildDfa [ { From = 0; Label = "a"; To = 1 }; { From = 1; Label = "a"; To = 1 } ] 0 [ 1 ]
...
let ``Arroyuelo: alternation a|b, both branches`` () =
    let nfa =
        nfaWithSources [ { From = 0; Label = "a"; To = 1 }; { From = 0; Label = "b"; To = 2 } ] [ 0 ]
...
let ``Arroyuelo: epsilon query returns identity`` () =
    let nfa = nfaWithSources [ { From = 0; Label = "a"; To = 1 } ] [ 0; 1 ]
...
let ``Kronecker: multiple sources, only one connects`` () =
    let nfa =
        nfaWithSources [ { From = 0; Label = "a"; To = 2 }; { From = 1; Label = "b"; To = 2 } ] [ 0; 1 ]
    ...
    let ``Belyanin and Kronecker identical with extended alphabet`` (d: RPQTestData) =
        if d.Sources.Length = 0 then
            ...
            if v = 0 then
                ...
                let alphabet = d.Edges |> List.map (fun t -> t.Label) |> Set.ofList |> Set.toList

                if List.isEmpty alphabet then
                    true
                else
                    let firstSym = List.head alphabet
                    let dfa = TestHelpers.buildDfa [ { From = 0; Label = firstSym; To = 1 } ] 0 [ 1 ]
                    let belyResult = BelyaninRPQ.evaluate dfa nfa
                    let kronResult = KroneckerRPQ.evaluate dfa nfa
