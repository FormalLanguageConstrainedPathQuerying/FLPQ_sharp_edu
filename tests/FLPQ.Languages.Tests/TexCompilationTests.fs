...
[<Fact>]
[<Trait("Category", "TeX")>]
let ``LL step stack TeX compiles with pdflatex`` () =
    ...
    for step in steps do
        ...
        Assert.Contains(@"\end{pNiceMatrix}", step.stack)
        Assert.Contains(@"\begin{pNiceMatrix}", step.input)
        Assert.Contains(@"\end{pNiceMatrix}", step.input)

[<Fact>]
[<Trait("Category", "TeX")>]
let ``LR step stack TeX compiles with pdflatex`` () =
    let g = Grammar.parseGrammar "S -> a S\nS -> a"
