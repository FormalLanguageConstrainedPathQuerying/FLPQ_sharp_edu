...
| Step visualizer passes `bendReciprocalEdges = false` to `GssTikz.toTikzFromSets` | GSS edges grow forward along the input (no reciprocal pairs), so the TikZ bend for overlapped edge pairs — used by the RPQ graph figures — does not apply; RNGLR and GLL figures are byte-identical to before task 282, S2 |
